...
async def weibanTask(user):
    ...
    try:
        ...
        for i in range(len(main.userProjectId)):
            ...
            write_to_task_file(id,f"开始进行 {realName} 的任务：{taskName[i]}")
            index = 1
            # 获取列表
            for chooseType in [2,3]:
                ...
                for courseId in main.getCourse(i, chooseType):
                    ...
                    write_to_task_file(id,f"{realName} 开始学习 {main.resourceNames[index]} {index} / {len(finishIdList)}")
                    ...
                    write_to_task_file(id,f"{realName} 完成学习 {main.resourceNames[index]}")
                    index += 1
        print(f"\033[0;31;32m{id} {realName} 的任务已完成\033[0m")
        write_to_task_file(id,f"\033[0;31;32m{id} {realName} 的任务已完成\033[0m")
